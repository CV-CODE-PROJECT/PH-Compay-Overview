...
import { ChevronDown, ChevronRight, Briefcase, Users, Search, LayoutGrid, List } from 'lucide-react';
...
export default function PositionGallery({ data, isLoading, onRowClick }: PositionGalleryProps) {
  ...
  const groupedData = useMemo(() => {
    ...
    const filtered = data.filter(item => {
      const searchStr = `${item.Position} ${item.Team}`.toLowerCase();
                ...
                {expandedTeams[team] && (
                  ...
                  >
                    <div className={`px-6 pb-6 pt-2 border-t border-slate-100 bg-slate-50/30 ${
                      ...
                      {groupedData[team].map((item, idx) => (
                        <motion.div
                          key={`gallery-item-${item._internalId || `${team}-${idx}`}`}
                          initial={{ scale: 0.95, opacity: 0 }}
                          animate={{ scale: 1, opacity: 1 }}
                          transition={{ delay: idx * 0.05 }}
                          onClick={() => onRowClick(item)}
                          className={`bg-white border rounded-xl p-4 cursor-pointer hover:border-blue-400 hover:shadow-lg hover:shadow-blue-500/5 transition-all group ${
                            viewMode === 'grid' ? 'flex flex-col gap-3' : 'flex items-center justify-between'
                          } ${item['Cấp bậc']?.toLowerCase().includes('manager') ? 'border-amber-100 bg-amber-50/10' : 'border-slate-200'}`}
                        >
                          <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                              item['Cấp bậc']?.toLowerCase().includes('manager') ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-500'
                            }`}>
                              <Users className="w-4 h-4" />
                            </div>
                            <div className="flex flex-col">
                              <span className="text-xs font-black text-slate-800 leading-none group-hover:text-blue-600 transition-colors uppercase tracking-tight">
                                {item.Position}
                              </span>
                              <span className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-widest leading-none">
                                {item['Cấp bậc'] || 'Standard'}
                              </span>
                            </div>
                          </div>
